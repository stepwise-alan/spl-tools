...
static  int sortcmp(__off_t st_size1 , __off_t st_size2 , __time_t st_atim_tv_sec1 , __time_t st_atim_tv_sec2 , __time_t st_ctim_tv_sec1 , __time_t st_ctim_tv_sec2, __time_t st_mtim_tv_sec1 , __time_t st_mtim_tv_sec2 , __mode_t st_mode1 , __mode_t st_mode2 , int strcoll12 , int strcmp12 , unsigned all_fmt )  {
  ...
  if ((sizeof(off_t) > sizeof(int ))) {
    if ((dif != 0)) {
      #ifdef CONFIG_LFS
      (dif = (1 | ((int ) (((uoff_t_0 ) dif) >> (8 * (sizeof(dif) - sizeof(int )))))));
      #else
      (dif = (1 | ((int ) (((uoff_t_1 ) dif) >> (8 * (sizeof(dif) - sizeof(int )))))));
      #endif
    }
  }
